...
const AdminSignIn = () => {
  ...
  useEffect(() => {
    // Handle authentication messages and errors
    if (auth.message) {
  ...
  useEffect(() => {
    // Redirect to admin dashboard when user is successfully logged in
    if (auth.user && auth.user.data) {
      // Check if the logged-in user is an admin
      if (["ADMIN", "ORGANIZER"].includes(auth.user.data.role)) {
        router.push(`/admin`);
      } else {
        // Logout and show error if not an admin
        auth.logout();
